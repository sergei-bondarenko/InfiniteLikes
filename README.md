...
Сначала нужно получить токен в vk.com – это шестнадцатиричное число длиной 85 знаков. Это что-то вроде временного пароля к аккаунту. Тот, кто его знает, может совершать разрешённые действия в аккаунте.
...
* Делать заказы выгоднее мелкими партиями. Если заказать 2 подписчика, обычно на страницу подписывается 2-6 пользователей. Большие заказы не так выгодны, на 30 заказанных подписчиков, подписывается обычно 30-35. Поэтому лучше делать 15 заказов по 2 подписчика, чем один на 30. Это просто осуществить с помощью небольшого скрипта save\_offers.js. Его нужно просто скопировать в консоль браузера, а затем там же, в консоли, вызвать функцию save\_offers с требуемыми параметрами, например, так: save\_offers("sub", "id123456", 2, 180) – заказывать по 2 подписчика на страницу id123456 каждые 180 секунд.
* Не выполнять задания с нескольких аккаунтов, но с одного ip адреса! Сервис likes.fm блокирует такие аккаунты. Если больше не приходят задания (во всех окошках написано "Новые предложения подгружаются, подождите.."), вероятнее всего, что likes.fm заблокировал этот аккаунт. Его можно восстановить, но только при наличии доступа к телефону, на который привязана страничка. Подробнее ответят в техподдержке сайта.
